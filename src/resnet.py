...
class ResenetUnet(nn.Module):
    def __init__(self, layer_num: int = 50, num_classes: int = 6):
        super(ResenetUnet, self).__init__()
        backbone_map = {34: resnet34, 50: resnet50, 101: resnet101}
        layer_map = {50: [2, 3, 5, 2], 101: [2, 3, 21, 2]}
        ...
        # self.down16 = IntermediateLayerGetter(self.backbone.layer3[-1].relu, {'relu': 'down16'})
        # self.down32 = IntermediateLayerGetter(self.backbone.layer4[-1].relu, {'relu': 'down32'})

    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        backbone_out = self.backbone(x)
        up1 = self.up1(backbone_out['down32'], backbone_out['down16'])
        up2 = self.up2(up1, backbone_out['down8'])
        up3 = self.up3(up2, backbone_out['down4'])
        up4 = self.up4(up3, backbone_out['down2'])
        up5 = self.up5(up4)
        double_conv = self.double_conv(up5)
        logits = self.out_conv(double_conv)
