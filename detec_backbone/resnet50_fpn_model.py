...
class Bottleneck(nn.Module):
    # 经过一个conv分支块，通道变为原来两倍（但相对这个conv分支块的第一个conv通道为四倍），图像尺寸变为1/2
    expansion = 4

    # 未使用mid_channel, 更具expansion来确定，（一个conv分支块里，第三个conv的通道为第一个的四倍）
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(3, self.in_channel, kernel_size=7, stride=2,
                               padding=3, bias=False)   # 第一个7*7的卷积，输入通道为3，输出通道为64， 图形尺寸变为 1/2
        self.bn1 = norm_layer(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # maxpooling , 图形变为1/4
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        downsample = None
        # 残差里，统一图像尺寸，resnet50中一般in_channel == channel * expansion ，只用下采样1/2, 第一个残差（虚线连接）
